...
- [ ] 用户完成本地复测
- [ ] 云端强随机密钥、AI配置、持久化路径已落实
- [ ] 生产依赖在Linux上安装成功，服务进程持续运行
- [ ] `/pdltk/`、`/pdltk/exam`、`/pdltk/chat`、`/pdltk/me`、`/pdltk/admin` 可直接访问并刷新
- [ ] `/pdltk/api/health` 返回200，正常抽题、交卷、AI调用、管理端访问通过
- [ ] 旧 `/api/payment/alipay/*` 和 `/api/sms/send` 返回404
- [ ] 首次启动只有题库数据，没有本地测试账户、订单或激活码
